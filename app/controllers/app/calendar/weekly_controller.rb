class App::Calendar::WeeklyController < App::BaseController
  def index
    set_dates
    @selected_date = params[:selected_date]&.to_date || Date.current
    @activities = Current.user.activities.where(started_at: @start_date..@end_date)
                    .group_by { |activity| activity.started_at.to_date.strftime("%Y-%m-%d") }
